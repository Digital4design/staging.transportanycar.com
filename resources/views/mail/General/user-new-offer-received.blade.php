...
<div style="font-weight: 300; font-family: 'Outfit', sans-serif; font-size: 16px; line-height: 22px; max-width: 600px; width: 100%; margin: auto; text-align: center; padding: 0; box-sizing: border-box;">
    ...
    <div style="background-color: #f1f1f1; padding: 20px; text-align: left;">
        ...
        <p style="font-family: 'Outfit', sans-serif; font-size: 16px; line-height: 20px; font-weight: 300; color: #000000; margin-bottom: 15px;">{{$data->message}}yes quote</p>
        <a  
        {{-- href="{{route('front.quotes', ['id' => $data->user_quote_id, 'type' => 'email'] )}}"  --}}
        href="{{ route('front.quotes', ['id' => $data->user_quote_id, 'user_id' => $data->getTransporters->id ?? null]) }}"

            style="background: #52D017; color: #ffffff; text-decoration: none; font-size: 16px; line-height: 20px; font-weight: 400; border: none; display: inline-block;  cursor: pointer; padding-top: 6px; padding-bottom: 6px; padding-left: 30px; padding-right: 30px;">Reply</a>
    </div>
