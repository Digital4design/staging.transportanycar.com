<style>
    .read-more {
        white-space: normal !important;
        max-width: 300px !important;
        word-wrap: break-word !important;
    }

    .user-feedback-stars li {
        padding: 0;
    }
</style>
<div class="overall-review py-1 py-md-1 mb-3">
    
    <?php
        $total_stars = 5; // Total number of stars
        $yellow_stars = floor($average_rating); // Full yellow stars
        $half_star = $average_rating - $yellow_stars >= 0.5; // Check for a half-star
        $grey_stars = $total_stars - $yellow_stars - ($half_star ? 1 : 0); // Remaining grey stars
    ?>
    
    

    <div class="total-review-count my-3"> Customer reviews</div>
    <ul class="review-count-bar">
        <?php $__currentLoopData = [5, 4, 3, 2, 1]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $rating): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <?php
                $percentage = max(0, $ratings['star_'.$rating] ?? 0); // Prevent negative values
            ?>
            <li>
                <span class="review-steps"><?php echo e($rating); ?></span>
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 19" fill="none">
                    <path d="M10 0L12.2451 6.90983H19.5106L13.6327 11.1803L15.8779 18.0902L10 13.8197L4.12215 18.0902L6.36729 11.1803L0.489435 6.90983H7.75486L10 0Z" fill="#595959"/>
                </svg>
                <div class="review-base-bar">
                    <div class="review-active-bar" style="width:<?php echo e(min(100, $percentage)); ?>%"></div>
                </div>
                <span class="review-percentage"><?php echo e(number_format($percentage, 0)); ?>%</span>
            </li>
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    </ul>
    
</div>
<div class="review-outer-wrap " >
    <?php $__currentLoopData = $feedbacks; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $feedback): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <div class="review-wrap">
            <div class="feedback-user-name">
                <?php if($feedback->quote_by_transporter->quote->user ?? ''): ?>
                    <?php if($feedback->quote_by_transporter->quote->user->name ?? ''): ?>
                        <?php echo e($feedback->quote_by_transporter->quote->user->name ?? ''); ?>

                    <?php else: ?>
                        <?php echo e($feedback->quote_by_transporter->quote->user->username ?? ''); ?>

                    <?php endif; ?>
                <?php else: ?>
                    <?php echo e($feedback->first_name); ?>

                <?php endif; ?>
            </div>
            <ul class="wd-star-lst user-feedback-stars other-reviews">
                <?php for($i = 1; $i <= $total_stars; $i++): ?>
                    <li>
                        <svg width="20" height="20" viewBox="0 0 12 12" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                            <path
                                d="M6 0L7.34708 4.1459H11.7063L8.17963 6.7082L9.52671 10.8541L6 8.2918L2.47329 10.8541L3.82037 6.7082L0.293661 4.1459H4.65292L6 0Z"
                                fill="<?php echo e($i <= $feedback->rating ? '#FFA800' : '#ccc'); ?>" />
                        </svg>
                    </li>
                <?php endfor; ?>
                <div class="feedback-user-verified">
                    <svg xmlns="http://www.w3.org/2000/svg" width="11" height="9" viewBox="0 0 11 9"
                        fill="none">
                        <path
                            d="M3.73608 8.04173L0.161084 4.46672C-0.0536948 4.25195 -0.0536948 3.90371 0.161084 3.6889L0.938883 2.91108C1.15366 2.69628 1.50192 2.69628 1.7167 2.91108L4.125 5.31935L9.28329 0.161084C9.49807 -0.0536948 9.84633 -0.0536948 10.0611 0.161084L10.8389 0.938905C11.0537 1.15368 11.0537 1.50192 10.8389 1.71672L4.51391 8.04175C4.2991 8.25653 3.95086 8.25653 3.73608 8.04173Z"
                            fill="#52D017" />
                    </svg>
                    <span>Verified</span>
                </div>
            </ul>
            <div class="font-weight-light"><?php echo e(general_date($feedback->date)); ?></div>
            <div class="feedback-item"><?php echo e($feedback->vehical_name); ?> </div>
            
            <div class="font-weight-light"><?php echo $feedback->comment ? readMoreHelper($feedback->comment, 200) : '-'; ?></div>
        </div>
    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
</div>
<?php if($feedbacks->lastPage() > 1): ?>
    <nav aria-label="...">
        <ul class="pagination">
            
            <li class="page-item<?php echo e($feedbacks->currentPage() == 1 ? ' disabled' : ''); ?>">
                <a class="page-link" href="<?php echo e($feedbacks->previousPageUrl()); ?>" rel="prev"
                    aria-label="<?php echo app('translator')->get('pagination.previous'); ?>">
                    <svg width="6" height="11" viewBox="0 0 6 11" fill="none"
                        xmlns="http://www.w3.org/2000/svg">
                        <path d="M5.5 1L1 5.5L5.5 10" stroke="black" stroke-linecap="round" />
                    </svg>
                </a>
            </li>

            
            <?php for($i = 1; $i <= $feedbacks->lastPage(); $i++): ?>
                <?php if($i == $feedbacks->currentPage()): ?>
                    <li class="page-item active" aria-current="page"><span class="page-link"><?php echo e($i); ?></span>
                    </li>
                <?php elseif($i >= $feedbacks->currentPage() - 2 && $i <= $feedbacks->currentPage() + 2): ?>
                    <li class="page-item"><a class="page-link"
                            href="<?php echo e($feedbacks->url($i)); ?>"><?php echo e($i); ?></a></li>
                <?php elseif(
                    ($i == $feedbacks->currentPage() - 3 && $i > 1) ||
                        ($i == $feedbacks->currentPage() + 3 && $i < $feedbacks->lastPage())): ?>
                    <li class="page-item disabled" aria-disabled="true"><span class="page-link">...</span></li>
                <?php endif; ?>
            <?php endfor; ?>

            
            <li class="page-item<?php echo e($feedbacks->currentPage() == $feedbacks->lastPage() ? ' disabled' : ''); ?>">
                <a class="page-link" href="<?php echo e($feedbacks->nextPageUrl()); ?>" rel="next"
                    aria-label="<?php echo app('translator')->get('pagination.next'); ?>">
                    <svg width="7" height="11" viewBox="0 0 7 11" fill="none"
                        xmlns="http://www.w3.org/2000/svg">
                        <path d="M1 1L5.5 5.5L1 10" stroke="black" stroke-linecap="round" />
                    </svg>
                </a>
            </li>
        </ul>
    </nav>
<?php endif; ?>
<?php /**PATH /var/www/html/public_html/resources/views/front/dashboard/partial/feedback_listing.blade.php ENDPATH**/ ?>